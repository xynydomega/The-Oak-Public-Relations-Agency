      ...
      <div className="max-w-6xl mx-auto w-full">
        <div className="flex justify-center mb-12">
          <motion.div 
            ...
            className="relative"
          >
            <div className="w-24 h-24 rounded-full border border-brand-gold/20 flex items-center justify-center bg-brand-gold/5 backdrop-blur-sm">
              <Trees className="w-12 h-12 text-brand-gold opacity-80" strokeWidth={1.2} />
            </div>
            {/* Decorative outer ring */}
            <div className="absolute inset-0 -m-3 border border-brand-gold/10 rounded-full animate-pulse" />
          </motion.div>
        </div>
        <h2 className="text-3xl lg:text-4xl font-lora font-bold text-center mb-20 text-balance text-[#EFEAE2]">
          ...
          {disciplines.map((item, idx) => {
            ...
            return (
              <motion.div 
                ...
                className="bg-[#2F4F3E] border-[3px] border-[#9C6B3E] p-10 flex flex-col items-center text-center group hover:bg-[#2F4F3E]/90 transition-all duration-500 rounded-[2.5rem] shadow-[0_0_40px_rgba(156,107,62,0.25)] hover:shadow-[0_0_60px_rgba(156,107,62,0.45)]"
              >
